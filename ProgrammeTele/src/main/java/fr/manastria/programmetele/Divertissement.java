...
public class Divertissement extends Emission {
    ...
    public Divertissement(String nom, String nomAnimateur) // constructeur de la classe Divertissement
    {
    ...
    // méthode Programmer : programmer l'objet dans la grille de programmes à l'heure indiquée en paramètre avec les contraintes spécifiques le la classe héritée
    public int Programmer(int heure)
    {
        int error;
        if ((heure < 18)|| (heure > 23)) //contraintes horaies non respectées => pas de programmation, génération d'une erreur 
        {
            return -1;
        } 
        else        //contraintes horaires respectées => programmation effectuée
        {    
